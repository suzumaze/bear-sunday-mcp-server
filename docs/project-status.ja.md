...
| `suzumaze/bear-sunday-mcp-server` | `v0.9.1` | contract coverage tool、任意のview、compactなtext fallback |
| MCP tool inventory | `v0.9.1`で24 tools | 日本語・英語manual整備済み |
| Contract coverage UI | 任意のMCP Apps view | read-only、structured/text fallbackを維持 |
| `bear-semantic` agent Skill | `v0.9.1`に同梱 | project diagnosticsとcontract導入workflowを含む |
