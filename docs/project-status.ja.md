...
flowchart TB
    ...
    M5 --> R1["core v0.1.9 / MCP v0.10.2<br/>一括インストール"]
...
| `suzumaze/bear-sunday-mcp-server` | `v0.10.2` | request discovery対応、Phpactor・BEAR拡張v0.1.9を同梱 |
| MCP tool inventory | `v0.10.2`で24 tools | 日本語・英語manual整備済み |
| Contract coverage UI | 任意のMCP Apps view | read-only、structured/text fallbackを維持 |
| `bear-semantic` agent Skill | `v0.10.2`に同梱 | project diagnosticsとcontract導入workflowを含む |
